"""
Nhánh: chong-overfitting
Mục tiêu: DÙNG RIDGE REGULARIZATION ĐỂ GIẢM OVERFITTING

Giữ NGUYÊN mọi thứ giống hệt nhánh main (cùng 17 biến đa thức, cùng 25 mẫu train,
cùng random seed) để so sánh công bằng — CHỈ khác duy nhất: thêm số hạng phạt
(regularization) vào công thức cập nhật trọng số.

Ý tưởng Ridge Regularization:
Hàm mất mát mới = MSE + lambda * (tổng bình phương các trọng số w)
=> mô hình bị "phạt" nếu hệ số w quá lớn, buộc nó phải đơn giản hơn,
   bớt "học thuộc" nhiễu, nhờ đó tổng quát hóa tốt hơn trên dữ liệu mới.
"""
...
TOC_DO_HOC = 0.01
SO_VONG_LAP = 60000
LAMBDA_REGULARIZATION = 50.0   # độ mạnh của "hình phạt" — càng lớn, mô hình càng bị ép đơn giản


# =====================================================
# BƯỚC 1: Đọc dữ liệu (giống hệt nhánh main)
# =====================================================
...
# =====================================================
# BƯỚC 2: Chia dữ liệu — CÙNG seed, cùng cách chia với nhánh main
# =====================================================
np.random.seed(42)
...
# =====================================================
# BƯỚC 4: Tạo biến đa thức — CÙNG hàm với nhánh main
# =====================================================
...
print(f"Số mẫu train: {SO_MAU_TRAIN} | Số biến đầu vào: {so_dac_trung}")
print(f"Lambda (độ mạnh regularization): {LAMBDA_REGULARIZATION}\n")


# =====================================================
# BƯỚC 5: Huấn luyện bằng Gradient Descent — CÓ Ridge Regularization
# =====================================================
trong_so = np.zeros(so_dac_trung)
he_so_chan = 0.0
...
for vong_lap in range(SO_VONG_LAP):
    ...
    # Đạo hàm gốc (giống nhánh main) + số hạng phạt regularization (2*lambda/m)*w
    # LƯU Ý: hệ số chặn (b) KHÔNG bị phạt, chỉ phạt các trọng số w
    dao_ham_w = (2 / SO_MAU_TRAIN) * (X_train.T @ sai_so) + (2 * LAMBDA_REGULARIZATION / SO_MAU_TRAIN) * trong_so
...
print("\n=== KẾT QUẢ: SO SÁNH TRAIN vs TEST (có Regularization) ===")
...
print(
    "\nNhận xét: so với nhánh main (không regularization), khoảng cách giữa "
    "train và test giờ nhỏ lại đáng kể — mô hình bớt 'học vẹt' tập train, "
    "nhờ đó kết quả trên dữ liệu mới (test) ổn định hơn, dù độ khớp trên train "
    "có giảm nhẹ. Đây chính là đánh đổi (trade-off) đặc trưng của regularization: "
    "hy sinh một phần độ khớp trên train để đổi lấy khả năng tổng quát hóa tốt hơn."
)
...
# Biểu đồ 3: Giá thực tế vs Giá dự đoán
...
plt.suptitle(f"Mô hình CHỐNG OVERFITTING — Ridge Regularization (lambda={LAMBDA_REGULARIZATION})")
plt.tight_layout()
plt.savefig("bieu_do_chong_overfitting.png", dpi=150)
print("\nĐã lưu biểu đồ vào file: bieu_do_chong_overfitting.png")
plt.show()
